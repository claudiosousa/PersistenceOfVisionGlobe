...
void loop(void){
  ...
  if(role == TX){
    ...
    unsigned long cycles = 1; //Change this to a higher or lower number. 
...
//      }
   ...
   if(!radio.txStandBy(1000)){ counter+=3; } //Standby, block only until FIFO empty or auto-retry timeout. Flush TX FIFO if failed
